        ...
        protected override void Update(GameTime gameTime)
        {
            ...
            if (startPathfinding == true)
            {
                ...
                if (loadPath.IsClicked(ms))
                {
                    ...
                    Vertex<(int, int)> start = graph.Search(((int)startTile.Pos.X, (int)startTile.Pos.Y));
                    Vertex<(int, int)> end = graph.Search(((int)endTile.Pos.X, (int)endTile.Pos.Y));
                    //vertex<vertex<int, int>> fixed  to <(int, int)>
                    
                    if (manhattan)
                    {
                        path = graph.AStarPF(start, end, Manhattan(start, end));
                    }
                    else if (diagonal)
                    {
                        path = graph.AStarPF(start, end, Diagonal(start, end));
                    }
                    else if (euclidean)
                    {
                        path = graph.AStarPF(start, end, Euclidean(start, end));
                    }
                }
